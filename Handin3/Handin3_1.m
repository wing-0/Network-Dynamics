...
[TIME,PATH,~] = graphshortestpath(sparse(Wl),1,17);
[M,~,~] = graphmaxflow(sparse(Wcap),1,17);

fprintf('--------------------------------------------------------------\n')
fprintf('Results a), b) \n')
fprintf('--------------------------------------------------------------\n')
fprintf('Shortest path node 1-17 \n')
fprintf('\t%-6s \n','Node')
fprintf('\t%-6.0f \n',PATH')
fprintf('Length %0.2f mi \n',TIME*60)
...
% Price of anarchy
del_uo = sum(f_uo.*l./(1-f_uo./Cap));
del_so = sum(f_so.*l./(1-f_so./Cap));
poa = del_uo/del_so;
...
fprintf('Price of anarchy (w/tolls) = %0.3f \n',poa)
fprintf('--------------------------------------------------------------\n')
...
% Construct tolls for Wardrop equilibrium coinciding with system optimum
